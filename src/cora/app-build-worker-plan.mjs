...
export function createApprovedAppBuildWorkerPlan(input = {}) {
  ...
  const changes = Object.freeze([
    Object.freeze({ kind: 'create', path: `generated-apps/${slug}/definition.json`, contentDigest: digest(definition) }),
    Object.freeze({ kind: 'create', path: `generated-apps/${slug}/contract.test.mjs`, contentDigest: digest({ route, componentCount: definition.components.length }) }),
  ]);
  return Object.freeze({ format: CORA_APP_BUILD_WORKER_PLAN_FORMAT, valid: true, tenantId, revisionReceiptId, approvalReceiptId: text(input.approval.receiptId, 'approval receipt', 256), workspaceProject: Object.freeze({ projectKey, sourceKind, defaultBranch }), changes, verification: Object.freeze({ required: Object.freeze(['schema-validation', 'tenant-scope-test', 'route-contract-test']), status: 'not_run' }), deployment: Object.freeze({ status: 'not_requested', revert: `remove generated-apps/${slug} before any commit` }), execution: 'not_performed', providerInvocation: 'not_performed', filesystemMutation: 'not_performed', publication: 'not_performed' });
}
