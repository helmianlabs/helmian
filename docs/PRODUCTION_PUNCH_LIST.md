...
| PROVIDER-001 | P1 | Verify the four Frontier provider adapters in Cloud | Provider selection -> server-side credential/profile lookup -> documented provider request -> redacted receipt/usage ledger -> UI result. Cloud currently lists `openai_codex`, `claude`, `gemini`, and `grok` at `src/cloud/oauth-connection-contract.mjs:11-16`, but every entry says `adapter: not_configured`; the contract returns `invocation: not_performed` and `tools: not_granted` at `src/cloud/oauth-connection-contract.mjs:36-49`. Live credentials and canaries are not proven. | ☐ OPEN |
