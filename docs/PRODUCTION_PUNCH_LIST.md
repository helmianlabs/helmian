...
| OAUTH-001 | P1 | Complete provider OAuth/API connection flows | Admin owner/admin -> `createCloudOAuthAuthorization` -> provider authority registry -> Gemini authorization URL -> PKCE token POST -> vault-only token handoff -> tenant `provider_connections` pending row. Source: `src/cloud/oauth-connection-contract.mjs`, `src/cloud/provider-oauth-authority.mjs`, `src/cloud/provider-oauth-flow.mjs`, and `src/cloud/provider-connection-repository.mjs`. Focused tests pass 13/13; Fly `helmian-cloud` release 46 is deployed, `/admin` is live 200, and protected `/api/admin/session` is live 403 without a session. Live Gemini client registration, callback route, production vault, and canary remain open; OpenAI Codex, Claude, and direct Grok API OAuth have explicit documented-authority blockers. | ☐ CODE SLICE DEPLOYED / LIVE EVIDENCE OPEN |
