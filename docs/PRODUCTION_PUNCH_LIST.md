...
| RBAC-001 | P1 | Prove ABAC/RBAC enforcement for Cora actions | Signed identity/tenant/role -> policy check -> allowed or denied action -> durable receipt -> UI evidence. The bounded workspace-project slice is code/local verified and deployed in Fly release `v49`: `src/cloud/live-admin.mjs:897-916` -> `src/cloud/tenant-action-authorization.mjs:17-88` -> `src/cloud/workspace-project-repository.mjs:30-42` -> `web/cloud-admin/app.js:937-944`; live owner/admin/member identity matrix and live Neon receipts remain open. | ☑ DEPLOYED / LIVE MATRIX OPEN |
