...
| LIVE-001 | P0 | Prove the deployed Helmian admin/auth surface | `helmian.cloud` -> Vercel external rewrites in `web/marketing/vercel.json:6-17` -> Fly `/admin` and `/api/admin` -> live UI 200, app asset 200, unauthenticated API 403, and PKCE login 302. The login redirect carries `redirect_uri=https://helmian.cloud/admin/auth/callback`. Source callback wiring now issues and verifies an HMAC-signed `hs1` session at `src/cloud/identity-gateway.mjs:45-68, 133-149`; external identity callback completion, deployed session secret, and Troy visual sign-off remain open. | ☑ ROUTE VERIFIED / SIGNED SESSION CODE VERIFIED / LIVE OIDC OPEN |
...
| RBAC-001 | P1 | Prove ABAC/RBAC enforcement for Cora actions | Signed identity/tenant/role -> policy check -> allowed or denied action -> durable receipt -> UI evidence. The bounded workspace-project slice is now code- and local-route verified: `src/cloud/live-admin.mjs:897-916` -> `src/cloud/tenant-action-authorization.mjs:17-88` -> `src/cloud/workspace-project-repository.mjs:30-42` -> `web/cloud-admin/app.js:937-944`; live owner/admin/member identity matrix and live Neon receipts remain open. | ☑ CODE/LOCAL VERIFIED / LIVE MATRIX OPEN |
