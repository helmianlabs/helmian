...
import { createLiveHelmianCloudAdminHandler, LIVE_ADMIN_WORKSPACE_LAYOUT_PATH, LIVE_ADMIN_WORKSPACE_LAYOUT_RESET_PATH, LIVE_ADMIN_WORKSPACE_ROLE_DEFAULTS_PATH } from '../src/cloud/live-admin.mjs';
...
function adminPool() { const client = { async query(sql) { const q = String(sql).replace(/\s+/g, ' ').trim().toLowerCase(); if (['begin', 'commit', 'rollback'].includes(q) || q.startsWith('select set_config')) return { rowCount: 0, rows: [] }; if (q.includes('from helmion.tenant_memberships')) return { rowCount: 1, rows: [{ tenant_id: 'customer-a', role: 'admin' }] }; throw new Error(`unexpected query ${q}`); }, release() {} }; return { connect: async () => client }; }
...
test('role defaults are owner/admin-only and remain Organization-derived', async (t) => {
  const calls = [];
  const repository = { async readRoleDefaults(actor) { calls.push(['read', actor]); return { roleDefaults: [{ role: 'member', layout: { density: 'comfortable' } }] }; }, async saveRoleDefault(actor, role, input) { calls.push(['save', actor, role, input]); return { updated: true, role, layout: input }; } };
  const admin = await createLiveHelmianCloudAdminHandler({ env, pool: adminPool(), identity: identity(), page: '<p>test</p>', script: 'void 0;', expectedMigrations: [], workspaceLayoutRepository: repository });
  const clm = await startCoraClm({ host: '127.0.0.1', port: 0, runTurn: async () => ({ text: 'ok', model: 'test' }), notifyBackgroundAgents: false, httpRequestHandler: admin.handler });
  t.after(async () => { await clm.close(); await admin.close(); });
  const base = clm.healthUrl.replace('/healthz', ''); const headers = { cookie: 'helmion_admin_session=active-session' };
  const read = await fetch(`${base}${LIVE_ADMIN_WORKSPACE_ROLE_DEFAULTS_PATH}`, { headers }); assert.equal(read.status, 200); assert.equal((await read.json()).roleDefaults[0].role, 'member');
  const body = { role: 'member', visibleShelves: ['chat', 'cora', 'prepare', 'artifact', 'governance'], panelOrder: ['chat', 'cora', 'prepare', 'artifact', 'governance'], density: 'compact', defaultEnvoyChannelId: null };
  const save = await fetch(`${base}${LIVE_ADMIN_WORKSPACE_ROLE_DEFAULTS_PATH}`, { method: 'PUT', headers: { ...headers, 'content-type': 'application/json' }, body: JSON.stringify(body) }); assert.equal(save.status, 200); assert.equal(calls[1][1].tenantId, 'customer-a'); assert.equal(calls[1][2], 'member');
  assert.equal((await fetch(`${base}${LIVE_ADMIN_WORKSPACE_ROLE_DEFAULTS_PATH}?plant_id=warehouse-1`, { headers })).status, 400);
});
