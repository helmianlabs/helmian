...
test('agent task panel model reports empty, replay, and not-performed states', () => {
  ...
  assert.equal(model.empty, false); assert.match(model.statusLabel, /replay receipt/); assert.equal(model.claimsUnavailable, true); assert.equal(model.completionUnavailable, true); assert.equal(model.execution, 'not_performed'); assert.equal(model.agentInvocation, 'not_performed');
  assert.match(agentTaskPanelModel({ receipts: [{ status: 'prepared', claimStatus: 'claimed', completion: { completionStatus: 'not_executed', execution: 'not_performed' } }] }).statusLabel, /claim and completion state is shown/);
});
...
test('Cora session history model exposes truthful empty and unknown usage states', () => {
  assert.equal(coraSessionHistoryModel({ sessions: [] }).empty, true);
  const unknown = coraSessionHistoryModel({ sessions: [{ phase: 'started', providerEvidence: false, actualCostMinor: null }] });
  assert.equal(unknown.empty, false);
  assert.match(unknown.providerEvidence, /No provider evidence/);
