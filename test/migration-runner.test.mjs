  ...
  async connect() {
    ...
    return {
      async query(sql, parameters = []) {
        ...
        if (String(sql).includes('console_navigation_intent_enabled')) {
          pool.executedSql.push('009_console_navigation_action_policy.sql');
          return { rowCount: 0, rows: [] };
        }
...
test('migration runner applies ordered migrations once and confirms durable commits', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ['009_console_navigation_action_policy.sql', true, 'committed'],
    ],
  );
  assert.deepEqual(
    pool.executedSql,
    [
      ...
      '009_console_navigation_action_policy.sql',
    ],
  );
  ...
  assert.equal(pool.transactions.filter((entry) => entry === 'commit').length, 18);
