...
function adapter(result) {
  const receipts = [];
  return { receipts, service: createApprovedKnowledgeExecutionAdapter({ knowledgeLookup: async (input) => { receipts.push({ lookup: input }); return result; }, receiptSink: async ({ receipt }) => { receipts.push({ receipt }); return { recorded: true }; } }) };
}
