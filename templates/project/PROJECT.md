...
<!-- Created by `helmion project init` on {{CREATED_DATE}}. This file is yours
...
| Current sprint | `sprints/{{SPRINT_ID}}/` |
| Status | not started |
...
4. `planning/STATE.md` — where the work actually stands, with a proof for
   every row.
5. `sprints/{{SPRINT_ID}}/handoff-prompt.md` — the task in front of you right
   now.
...
| `sprints/{{SPRINT_ID}}/` | one bounded, falsifiable unit of work | the architect writes it, the builder executes it |
